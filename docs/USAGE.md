...
**Cross-device.** Desktop, phone, and browser pair against the same FastAPI sync service (`sync_api/`) with one-shot six-digit codes / QR. Tasks, notes, routine times, focus state, emergency-stop requests, and phone screen time flow both ways. The Android Today screen exposes the current alarms, quick Note/Todo capture, full Tasks/Notes access, and a Now section for active nudges. The web app at `{api}/web` is a single self-contained file - Three.js particle background, GSAP motion, full task/note/activity views.
